...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  describe('config dump', () => {
    ...
    it('composes the required raw overlay directly over the base', async () => {
      // A file at the old personal path is inert: an overlay reaches a launch
      // only by being named.
    ...
    it('keeps the Web overlay and ignores a file at the old personal path', async () => {
      writeFileSync(join(home, 'config.yaml'), [
        '- id: agent-loop',
        '  config:',
        '    agents:',
      ...
      expect(stdout).not.toContain('unnamed-provider')
    }, 30_000)
  })
})
