...
interface DumpConfigInvocation {
  mode: 'dump-config'
  surface: 'config' | 'web'
  /** Omit every caller layer and print the shipped tree. */
  defaultOnly: boolean
...
/**
 * Headless one-shot: `dsh -p "task"`. `--config` means what it means for every
 * other surface, so an automated run can name its composition instead of
 * depending on whatever the machine happens to hold.
 */
...
interface WebInvocation {
  mode: 'web'
  /** Overlay applied over the shipped Web composition. */
  config?: string
  host?: string
  port?: number
  dev: boolean
