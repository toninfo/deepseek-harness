...
The named file is applied directly over [`config/base.cordis.yml`](../config/base.cordis.yml) through the Include plugin's patch algorithm. It is not a complete replacement tree, and no surface overlay is added. The base deliberately contains no startup agent or interaction front door; the required overlay selects those deployment details. Relative config paths resolve from the invoking directory. A parse, schema, resolution, or plugin boot failure is reported and exits nonzero. SIGINT and SIGTERM dispose the mounted root before exit.
...
`dsh web` boots `base.cordis.yml` plus [`config/web.cordis.yml`](../config/web.cordis.yml), followed by an explicit `dsh web --config <path>` patch list when given. Nothing is discovered on disk: an overlay reaches a launch only by being named. `--host`, `--port`, `--workspace-root`, and repeatable `--trusted-host` values become Web host patches; their owning plugin schemas validate them at boot. `--dev` mounts the client-plugin HMR receiver and expects a separate `pnpm run dev:web` watcher for no-refresh client bundle updates.
...
`dsh -p "task"` uses the same base and Web composition, starts its Web host on an OS-assigned port, runs one fresh persisted session, prints the final answer, and exits. It takes `--config <path>` like every other surface — an automated run names its composition rather than depending on machine state — and rejects the raw config-dump flags.
...
Both modes treat the invoking directory as the default workspace root, load applicable `AGENTS.md` or `CLAUDE.md` instructions with a 65,536-byte render budget, and use an in-memory SQLite session content index. Each reads its overlay once at startup. The [app-boot Harness-home contract](../../../packages/ui/app-boot/README.md#the-harness-home) owns environment layering and `$DSH_HOME` resolution; the [credentials provider](../../../packages/credentials/credentials-local/README.md) owns credential storage.
...
The base mounts the native DeepSeek adapter, settings and credential providers, stable `web_search`, repository Plugin support, and session telemetry. Provider credentials live in `$DSH_HOME/.credentials.yaml` or the environment and remain rotatable because the launcher never hoists that document into `process.env`; `$DSH_HOME/.env` is the user's ordinary environment layer, beneath the invoking directory's `.env` and the inherited environment. Search uses `DEEPSEEK_API_KEY` and accepts `DEEPSEEK_SEARCH_BASE_URL`; `web_fetch` is disabled unless an overlay inserts a provider and enables it.
...
The empty `repository-plugins` row lets any named overlay mount prepared immutable repository Plugin generations. See the [repository Plugin contract](../../../packages/cordis/repository-plugin/README.md#standalone-app-configuration). The CLI also ships `@deepseek-ai/dsh-mcp-client` as a dependency for overlays, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.
