...
- 模型请求恢复：`dsh-llm-retry` 监听 `agent/request-error`，使用确切提供方的 normal 或无界策略，并发出不进入表层的 `llm/retry` 状态事件
