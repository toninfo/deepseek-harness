...
- Every package here sets `private: true` and omits `publishConfig`; the dsh release family excludes this directory.
...
- Promotion moves a package to its product-role group without renaming its `@deepseek-ai/dsh-*` package. Review its public contract, limitations, test evidence, release payload, runtime dependents, and named stable owner before promotion.
