...
export interface ApiRemoteAgentOptions {
   ...
   * publication. Keyed by the resumed session's own header because what a
   * Host installs may depend on what that session recorded: an agent preset
   * fixes the tools its history was produced under, so rebuilding it under
   * another composition would replay tool calls the agent can no longer make.
   * @param meta - the resumed session's persisted header.
   * @returns the Agent-scope setup to run before publication.
   */
  readonly setup?: (meta: SessionHeader) => AgentSetup | Promise<AgentSetup>
}
    ...
    if (resume === undefined) {
      resume = (async () => {
        try {
          ...
          // Built from the header before the published re-checks below, so
          // those stay adjacent to `resume` and a Host setup that awaits
          // (composing a preset, say) does not widen the collision window.
          const setup = options.setup === undefined ? undefined : await options.setup(inspected.meta)
