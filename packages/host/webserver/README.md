...
- **No server-wide TLS or origin policy** — composing applications register guards ([`dsh-host-access-gate`](../access-gate/README.md) is the shipped shared-secret gate). Route owners such as `dsh-client-connection` still enforce their own request policy. A non-loopback bind still exposes the TCP listener; put a TLS reverse proxy in front of a public bind.
