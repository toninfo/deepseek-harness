...
- **不提供服务器级 TLS 或来源策略**：组合层通过 `registerGuard` 注册拦截器（随附的共享密钥门是 [`dsh-host-access-gate`](../access-gate/README.zh.md)）。`dsh-client-connection` 等 route owner 仍会实施自己的请求策略。绑定非回环地址仍会暴露 TCP 监听器；公开绑定前应加 TLS 反向代理。
