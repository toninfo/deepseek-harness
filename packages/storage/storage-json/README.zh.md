...
无。该后端不贡献提示词、工具或 schema；它在 `ctx.storage` 后面持久化非会话领域数据，只供主机侧消费方使用。
...
- Windows 持久性依赖 libuv 的 `rename()`（使用替换的 `MoveFileExW`），没有显式 write-through 标志；append-log facet 落地时，计划把会话日志后端更严格的 Win32 write-through 发布辅助函数下移到此处（见 Agent Note 的迁移章节）。
