...
- **全接口绑定需要密钥**：没有至少 16 个字符的 `DSH_ACCESS_SECRET` 时，`--host 0.0.0.0` 会失败；该环境变量为空时，access-gate 行在回环上仍为空操作。
