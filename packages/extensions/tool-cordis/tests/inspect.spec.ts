...
describe('cordis_inspect', () => {
  it('reports all six sections by default', async () => {
    ...
    // cordis tools.
    expect(report).toContain('- tools (provided by ToolRuntime)')
  ...
  it('renders the api section from the generated catalog intersected with the LIVE runtime', async () => {
    ...
    expect(report).toMatch(/not running \(loadable services with no live provider\): .*shell/)
    ...
    expect(report).not.toContain('export interface ShellRunResult')
  ...
  it('fails loud for incompatible, unknown, and non-running names', async () => {
    ...
    const nonRunning = await call(ctx, 'cordis_inspect', { what: 'api', name: 'shell' })
    expect(nonRunning.isError).toBe(true)
    expect(text(nonRunning)).toContain('catalogued service "shell" is not running')
