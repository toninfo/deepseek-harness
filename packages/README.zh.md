...
`packages/<group>/<pkg>/` 包含 `@deepseek-ai/dsh-<pkg>` 包。**每个组 README 是规范的包／ctx 键映射。**
...
| [`attachment/`](attachment/README.md) | 持久附件 | 产品：稳定表面 |
...
组用于区分产品与支持基础设施。包加入现有组；新组则更新其 README 和此表。
