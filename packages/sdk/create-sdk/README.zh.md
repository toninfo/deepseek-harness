...
用于 `npm create @deepseek-ai/sdk [directory]` 的交互式初始化器。目录／名称／描述都提供可见且可编辑的默认值。树形选择器用于选择功能；有限选项通过 Right／Left 导航配置，只有选中相应选项后才会询问密钥文本。本地插件创建提供 none／plugin／tool 三选一。
