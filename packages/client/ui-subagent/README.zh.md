...
Web subagent 功能 owner：向 `conversation.session.header.actions` 贡献可懒加载展开的目录树，并向会话编辑器链贡献按原因区分的只读替代呈现。由 Host 支撑的组合 `@file` 与 `@session` source 属于 [`ui-reference`](../ui-reference/README.md)；本包不注册 slash source。
...
间接影响模型体验：本包的可继续 child 编辑器会触发 Host `subagent.prompt` RPC；被接纳的继续交互内容会成为普通 FIFO 用户消息，而浏览目录、导航 child 与查看持久化 transcript 都不会添加提示词 section 或模型 token。

#### KV Cache 影响

仅追加。用户后续消息只会向对应的新用户消息增加 token；目录与 transcript 操作增加零 token，且本包绝不改写更早的请求 token。
