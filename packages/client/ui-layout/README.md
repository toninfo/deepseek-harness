...
This package provides the Web GUI's three-column AppFrame, edge-column widths, and `ctx.layout` presentation control. The right column concedes space before the center; its occupant renders fullscreen while the frame retains the wide-screen track underneath. A closed sidebar retains a 56px control rail above the phone breakpoint; below `SIDEBAR_PHONE` (560px) a closed sidebar contributes zero grid width, AppFrame shows a top-left open control, and an open sidebar paints as an overlay drawer with a dismiss mask. The theme presenter owns color scheme, alias tokens, content font size, and document metadata. Layout state resets on reload.
...
- **Extremely narrow windows** — after the right panel closes, the center may still fall below 400px; above the phone breakpoint the left 56px rail remains, while below 560px a closed sidebar contributes zero grid width.
