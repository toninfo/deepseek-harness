...
使用本包可为 dsh Web 客户端提供 Settings 面板、连接恢复控件、由功能包贡献的导航，以及依次进行的首次运行引导。用户可以从侧边栏打开面板、立即重试失败的连接，并在宿主为回环浏览器提供本地配置文件时访问该文件。各功能包提供自己的设置行、分区和引导步骤；本包提供共享的界面展示，但不添加引导文案或「通用」分区的内置行。视口低于 560px 时，模态面板改为上方横向可滚动分区导航、下方内容区，避免选项行被挤成单字列。
...
宿主端在用户设置 seam 中注册 `ui-onboarding`，使仍携带该分节的已存设置文档继续有效；外壳本身仍不持有产品策略。
