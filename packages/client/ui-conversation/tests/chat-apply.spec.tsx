...
describe('apply wiring', () => {
  ...
  it('mounts the bash sample, the read row, and the todo row as keyed entries through the load-order seam', async () => {
    const b = await bench()
    ...
    expect(entries.map(e => e.options.key)).toEqual(['bash', 'read', 'todo_write'])
