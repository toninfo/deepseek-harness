...
describe('apply wiring', () => {
  ...
  it('mounts the bash sample, the read row, the web rows, and the product rows as keyed entries through the load-order seam', async () => {
    const b = await bench()
    ...
    expect(entries.map(e => e.options.key)).toEqual(['bash', 'read', 'web_search', 'web_fetch', 'todo_write', 'ask_user_question'])
