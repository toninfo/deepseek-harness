...
function buildAlphaLog(): SessionEvent[] {
  ...
  // The todo_write sample feeds both the TodoRow in the flow and the
  // todo/write snapshot event in the TodoPanel plan strip.
  const fixtureTodos = [
  ...
  // Turn 71: multimodal image sample — image blocks in a user message and an
  ...
  push({ type: 'turn/start', data: { turn: 71, trigger: { kind: 'message', source: { kind: 'user' } } } })
  push({
    type: 'user/message',
    surfaceOp: 'append',
    ...
    sessions: {
      ...
      prompt: (request) => {
        ...
        if (mode === 'steer' && replays.has(id)) {
          ...
          append(id, { type: 'steering/message', surfaceOp: 'append', data: { turn, message: userMessage(durable) } })
