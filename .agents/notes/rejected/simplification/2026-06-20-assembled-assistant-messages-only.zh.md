...
ACP `session/load` 可以将先前的 assistant 消息作为完整内容块回放，而非模拟原始的 token 流。加载后的 transcript（文本记录）无需重现每一个历史 delta；它必须展示相同的已完成 assistant 内容，并以有效的提供方历史恢复运行。
...
- 存储的日志大幅缩小，且在没有分片缺口的情况下保持 `seq` 连续。
