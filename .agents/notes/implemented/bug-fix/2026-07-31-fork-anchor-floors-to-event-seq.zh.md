...
apiproxy 的 fork 用例固定了 host 这一侧的约定：落在被中止轮次内的取整锚点会将该轮纳入切分结果，并以该轮初始化子会话。
...
从已停止的轮次 fork 会得到一个初始化内容截至该轮 `turn/end` 的子会话。被冻结的残缺文本是从分片事件重建出来的，从未成为 `assistant/message`，因此它不会进入子会话的模型 transcript（文本记录）——正如源会话恢复时它也不会进入一样，子会话拿到的上下文与源会话一致。
