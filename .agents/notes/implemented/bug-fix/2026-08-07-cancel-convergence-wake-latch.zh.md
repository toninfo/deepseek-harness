...
`signal.aborted` 判别项是承重的：它区分「中断前已排队的工作」——`keepInbox` 将其停放以待后续唤醒（`keepInbox` 停放契约）——与「abort 后显式的唤醒」，后者必须在收敛后执行。
...
**对每个非 idle 唤醒无条件锁存。** 不予采用：中断前的唤醒会在 `keepInbox` 取消后自动启动，违反 `keepInbox` 停放契约；「停放排队工作」测试与错误窗口的 steering 测试双双失败。
