...
- Extent equality is asserted, not assumed. It is the premise that turns "mirror the offset" from correct into subtly wrong, and it failed twice before this change: for the trailing-newline shape, and for any soft-wrapped draft on a platform whose scrollbar consumes width.
- The composer's text column is 8px narrower on every platform, including those that never draw a space-consuming scrollbar. That is the price of one geometry instead of a per-platform one, and it is paid in the layer metrics rather than in a conditional.
