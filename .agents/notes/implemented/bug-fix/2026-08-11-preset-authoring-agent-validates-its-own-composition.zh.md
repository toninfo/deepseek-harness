...
指导保留 `${DSH_HOME:-$HOME/.dsh}/.agent-presets/` 作为「我的 preset 在哪」的答案，同时把 agent 实际读取或编辑的路径改走 `list()` 或 `resolve()`。写出该路径对人讲是对的，喂给文件工具是错的：没有任何调用会报告根目录，`authorable` 只回答是否存在可写根，而 `list()` 无法揭示一个尚且为空的用户根。
