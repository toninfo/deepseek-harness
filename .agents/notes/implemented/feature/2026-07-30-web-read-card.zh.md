...
`ToolResultView` 多了第四个成员。在 `card` 上做穷尽 switch 的消费方需要新增 `read` 分支；generic/default 消费方可以使用卡片的 `content` 字段。Web 前端通过[读取卡片前端](2026-07-30-web-read-card-frontend.md)，从 `lines`/`lang`/`totalLines` 渲染带行号、语法高亮的视图。
