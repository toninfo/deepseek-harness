...
The [named-instance decision](2026-08-18-product-subagent-named-instances.md) currently allows multiple Claude Code rows, while Codex retains its single default name. Each additional Claude Code host provider row has its own `providerName`, and each exposed preset tool row binds that exact name through `provider` while keeping a unique `toolName`; the foreground/background scheduling choice does not constrain the number of supported instances.
