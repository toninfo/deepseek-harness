...
Web UI 的 HTTP 载体没有认证。`dsh web --host 0.0.0.0` 被拒绝，因为全接口绑定会把远程代码执行（`session.prompt` 驱动 bash）暴露给任何能打开该源的主机。因此通过公网用手机访问没有受支持的路径：`/api` 的 Host 栅栏是混淆代理人防御（[浏览器信任边界](../architecture/2026-07-28-api-browser-trust-boundary.zh.md)），不是登录，而特权方法仍只限回环。
