...
`dsh web` 默认绑定 `127.0.0.1`。CLI（命令行界面）仅在 `DSH_ACCESS_SECRET` 去除空白后至少 16 个字符时接受 `--host 0.0.0.0` 作为显式启用的全接口模式（[Web 访问门 Agent Note](./2026-08-18-web-access-gate.md)），并拒绝其他取值，使网络模式保持为一份规模小、经过审慎限定的约定。全接口模式仍然输出本机环回 URL，并在可用时输出第一个外部 IPv4 URL。
...
`dsh web` 的本地启动仍可通过 `http://127.0.0.1:3080` 访问；其他机器上的浏览器必须使用 `dsh web --host 0.0.0.0` 以及足够长的 `DSH_ACCESS_SECRET` 显式启用。CLI 尚未开放自定义接口地址或 IPv6 模式，而以编程方式使用承载层的消费方仍保留这种灵活性。服务器测试将环回模式和全接口模式向 Node 监听边界的传递固定为约定，Web 冒烟测试继续覆盖默认 CLI 路径。
