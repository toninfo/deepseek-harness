...
**Always 303 to `/` after a successful form login.** Rejected: that drops a Connection browser launch `?token=` that arrived on the denied URL, so the second cookie exchange fails and the browser asks the user to reopen the CLI URL.

**Resume from the `Referer` header.** Rejected: browsers omit or strip Referer under `Referrer-Policy`, and a forged Referer is not a reliable open-redirect defense compared with a form field that is sanitized to a same-origin relative path.

## Consequences

A phone can open the public origin, enter the secret, and use the ordinary session UI, including `session.prompt`. When the CLI-printed Connection browser URL carries `?token=`, the gate preserves that query across the password POST so `authorizeIndex` can still mint `dsh-auth-*` after `dsh_access` is set; the two cookies remain separate principals. Settings, credentials, native pick/open, and agent-preset authoring stay loopback-only. A public bind without a TLS reverse proxy leaks the secret and cookie on the wire. The guard registers in `apply` after the webserver already listens, so a request in that boot window can miss the gate. The Host fence and the access gate remain separate: a public Host still needs `--trusted-host` (or a derived LAN IP) in addition to the cookie.
