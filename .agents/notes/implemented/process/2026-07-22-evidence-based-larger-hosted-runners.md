...
**Keep static gates and post-build consumers on one runner.** Reusing one build avoids a setup wave, but build-duration variance delays every consumer and leaves their lint and snapshot tails after the static result. Independent jobs repeat the build while keeping both complete paths within the observed target.
...
GitHub rounds each larger-runner execution up to a whole minute, so complete-job measurement exposes both billed time and workflow complexity. Splitting Linux repeats setup twice and build once, but isolates coverage, static gates, and post-build consumers from each other's critical paths; consolidating Windows avoids repeating its slower setup.
