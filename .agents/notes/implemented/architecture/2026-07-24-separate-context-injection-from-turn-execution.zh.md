...
如果提示词准入被阻止或失败，调用方暂存的 next-step 输入会留在 outbox 中，供 `retry()` 或后续获准提示词使用；取消或 dispose（资源释放）可能丢弃这些输入。钩子产生的 `additionalContexts` 属于被拒绝的准入决策，因此永远不会落入日志。
...
- 被提示词准入阻止的消息不会打开轮次，也不会追加提示词或钩子产生的额外上下文；调用方暂存的 next-step 输入仍可重试。
