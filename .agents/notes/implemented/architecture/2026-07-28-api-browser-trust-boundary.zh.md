...
两条边界刻意留在本栅栏范围之外：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制；共享密钥登录门是 [`dsh-host-access-gate`](../../../../packages/host/access-gate/README.md)（[Web 访问门 Agent Note](../feature/2026-08-18-web-access-gate.md)）——这道栅栏是混淆代理人防御，不是那道门。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- Host 栅栏仍然不做认证。对公网绑定还需要访问门和 TLS 反向代理；特权 `/api` 方法仍只限回环，因为共享密钥不是操作者身份。
