...
当 session 配套插件附加到已有或已播种的会话时，它回放不可变日志以重建跟踪状态。服务为每项贡献提供一个可 dispose（资源释放）的子 fiber，因此轮次中途热重载是安全的，同时不赋予诊断逻辑对会话存储的所有权。
...
- `dsh-invariants` 配置全局启用状态以及包 allow/block regex 列表；每项检查仍由其产品包拥有并测试。
