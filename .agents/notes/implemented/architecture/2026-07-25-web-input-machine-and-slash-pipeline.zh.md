...
- 发送统一在 hub defaultSink：乐观清除 draft 和已提交图片后只走 `session.prompt {mode:'queue'|'steer'}`；失败时恢复已提交图片标识符，仅当 live draft 仍为空时才回填文本，因此不会覆盖飞行期间新增的文本或图片。不存在 Draft materialize 或 attach 事务。
