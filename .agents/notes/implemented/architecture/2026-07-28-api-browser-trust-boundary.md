...
Two boundaries stay deliberately out of scope of this fence: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and the shared-secret login gate is [`dsh-host-access-gate`](../../../../packages/host/access-gate/README.md) ([web access gate Agent Note](../feature/2026-08-18-web-access-gate.md)) — the fence is a confused-deputy defense, not that gate. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- The Host fence still does not authenticate. A public bind also needs the access gate and a TLS reverse proxy; privileged `/api` methods stay loopback-only because a shared secret is not an operator identity.
