...
前端与 Client plugin 产物缺失时，启动会失败，诊断信息会指示用户运行 `pnpm run build`。启动器不会验证产物是否为最新：已有的过期前端或 Client plugin 组合包仍会被接受，并可能继续运行旧版浏览器代码，直至下次构建。开发 Client plugin 时，`pnpm run dev:web` 会保持这些组合包为最新状态，并启用其热重载路径。
