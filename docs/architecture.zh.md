...
会话日志是权威依据。`deriveMessages()` 投影出模型历史；`assistant/chunk` 保证回放和 UI 保真。fork、恢复、transcript（文本记录）、遥测和持久化均派生自该日志。

**模型可见 ⟺ 已持久记录**：在 `step/start` 之前，循环会将当前运行时上下文快照作为一条带来源的 `user/message` 追加，随后对消息制作快照。这些消息、不可变附件的日志引用与折叠后的 `request/header` 共同重建每个请求。它会标记适配器默认值，使提议丢弃这些值并重新解析路由，同时不丢失显式设置。`dsh-agent-loop/invariant` 通过 `ctx.invariants` 断言可重建性，附件后端则断言附件字节完整性（[决策](../.agents/notes/implemented/architecture/2026-07-05-reconstructable-requests.md)）。
