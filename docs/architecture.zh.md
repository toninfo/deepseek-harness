...
**模型可见 ⟺ 已持久记录**：`step/start` 消息、请求头中的会话前缀和折叠后的 `request/header` 共同重建每个请求，日志中记录的附件引用通过经过完整性校验的不可变对象解析；`dsh-agent-loop/invariant` 通过 `ctx.invariants` 断言请求重建，附件后端断言字节完整性（[决策](../.agents/notes/implemented/architecture/2026-07-05-reconstructable-requests.md)）。
