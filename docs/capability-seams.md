...
| `ctx.sessionReferences` | `core` | [`session-reference`](../packages/context/session-reference) | - | `apiproxy` | - | Projects bounded current-surface conversation snapshots into durable untrusted message context; Host adapters own mention syntax. |
