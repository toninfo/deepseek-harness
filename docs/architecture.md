...
**Model-visible ⟺ durably recorded**: `step/start` messages plus the header's session prefix and folded `request/header` reconstruct every request, and logged attachment references resolve through integrity-checked immutable objects; `dsh-agent-loop/invariant` asserts reconstruction through `ctx.invariants`, attachment backends byte integrity ([decision](../.agents/notes/implemented/architecture/2026-07-05-reconstructable-requests.md)).
